...
def check_group_exists(brand_name: str):
    ...
    r = session.get(url_groups, params=params)
    if r.status_code not in (200, 201):
...
def create_group(brand_name: str, account_uid: str):
    ...
    r = session.post(url_post, json=payload)
...
def patch_group_users():
    ...
    r = session.patch(url_patch_users, json=payload)
...
def patch_group_roles(group_uid: str, account_uid: str):
    ...
    r = session.patch(url_patch_roles, json=payload)
    if r.status_code not in (200, 201, 204):
        raise RuntimeError(f"Patch roles failed: {r.status_code} - {r.text}")
    # If there's a JSON body return it, else return an empty dict
    try:
...
for brand_name in brand_names:
    ...
    if groups:
        # One or more matching groups found; assign roles to each
        for g in groups:
            # group may use 'id' or 'uid' key; be flexible
            ...
            # Try to extract account from group object if present (fallback to account_uid from hierarchy)
            ...
            # Patch roles for existing group
            try:
                roles_resp = patch_group_roles(group_uid, existing_account_uid)
                # roles_resp may be {} or JSON; just report success
                print(f"🔑 Roles patched for existing group {group_uid}. Response: {roles_resp if roles_resp else '(no content)'}")
            except Exception as e:
                print(f"❌ Failed to patch roles for existing group {group_uid}: {e}")
        # after processing existing groups, skip create step
        continue

    # Step 3: Create new group
    try:
        group = create_group(brand_name, account_uid)
        # created group may contain 'uid' or 'id'
